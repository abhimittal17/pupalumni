import React, { useRef, useEffect } from "react";
import { Link } from 'react-router-dom';
import Slider from "react-slick";
import VC from "../images/VC.jpg";
import './Accordion.css'
// import campus from "../Videos/campus.mp4";
import pup from "../static/pup.mp3"
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";

import { useParams } from "react-router-dom";
// import promotion from "../Videos/promotion.mp4";

import { BiChevronDown } from "react-icons/bi";
import a2  from "../images/A2.jpeg";
import a1 from "../images/A1.jpeg";
import a3 from "../images/A3.jpeg";
import v2 from "../images/v2.jpeg";
import v3 from "../images/v3.jpeg";
import v4 from "../images/v4.jpeg";
import v5 from "../images/v5.jpeg";
import v6 from "../images/v6.jpeg";
import v7 from "../images/v7.jpeg";
import v8 from "../images/v8.jpeg";
import v9 from "../images/v9.jpeg";
import v10 from "../images/v10.jpeg";
import v11 from "../images/v11.jpeg";
import v12 from "../images/v12.jpeg";
import a4 from "../images/A4.jpeg";
const features = [
  { description: '1. ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਸਾਬਕਾ ਵਿਦਿਆਰਥੀਆਂ ਵਿੱਚ ਭਾਈਚਾਰਕ ਸਾਂਝ ਅਤੇ ਭਰਾਤਰੀਭਾਵ ਦੀ ਭਾਵਨਾ ਪੈਦਾ ਕਰਨ ਲਈ ।' },
  { description: '2. ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਵਿਕਾਸ ਲਈ ਫੰਡਾਂ ਦਾ ਪ੍ਰਬੰਧ ਕਰਨਾ ਅਤੇ ਫੰਡ ਇਕੱਠੇ  ਕਰਨਾ ।' },
  { description: '3. ਸਮਾਜਿਕ ਅਤੇ ਸੱਭਿਆਚਾਰਕ ਸਮਾਗਮਾਂ ਦਾ ਪ੍ਰਬੰਧ ਕਰਨਾ ।' },
  { description: "4. ਆਮ ਤੌਰ 'ਤੇ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਦੇ ਹਿੱਤਾਂ ਨੂੰ ਅੱਗੇ ਵਧਾਉਣ ਲਈ ।" },
  { description: '5. ਅੱਜ ਦੀਆਂ ਵਿਦਿਅਕ, ਸੱਭਿਆਚਾਰਕ, ਸਮਾਜਿਕ ਅਤੇ ਅਕਾਦਮਿਕ ਸਮੱਸਿਆਵਾਂ ਬਾਰੇ  ਵਿਚਾਰਾਂ ਦੇ ਆਦਾਨ-ਪ੍ਰਦਾਨ ਲਈ ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਸਾਬਕਾ ਵਿਦਿਆਰਥੀਆਂ ਨੂੰ ਇੱਕ ਮੰਚ ਪ੍ਰਦਾਨ ਕਰਨਾ।' },
  { description: '6. ਐਸੋਸੀਏਸ਼ਨ ਦੇ ਉਦੇਸ਼ਾਂ ਦੀ ਪੂਰਤੀ ਲਈ ਸਾਹਿਤ, ਪੇਪਰ ਅਤੇ ਰਸਾਲੇ ਪ੍ਰਕਾਸ਼ਿਤ ਕਰਨਾ।' },
  { description: '7. ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਸਾਬਕਾ ਵਿਦਿਆਰਥੀਆਂ ਦੇ ਹਿੱਤਾਂ ਦੀ ਦੇਖਭਾਲ ਲਈ।' },
  { description: '8. ਐਸੋਸੀਏਸ਼ਨ ਦੀਆਂ ਸ਼ਾਖਾਵਾਂ ਖੋਲ੍ਹਣ ਲਈ।' },
]
const vcs = [
  { image: v11, name: "ਭਾਈ ਜੋਧ ਸਿੰਘ", year: "(1962-1965)", },
  { image: v6, name: "ਸਰਦਾਰ ਕਿਰਪਾਲ ਸਿੰਘ ਨਾਰੰਗ", year: "(1965-1975)", },
  { image: v3, name: "ਇੰਦਰਜੀਤ ਕੌਰ ਸੰਧੂ", year: "(1975-1977)", },
  { image: v4, name: "ਡਾ. ਅਮਰੀਕ ਸਿੰਘ", year: "(1977-1979)", },
  { image: v12, name: "ਡਾ. ਭਗਤ ਸਿੰਘ", year: "(1979-1982, 1986-1989)", },
  { image: v5, name: "ਡਾ. ਐਸ. ਐਸ. ਜੌਹਲ", year: "(1983-1986)" },
  { image: v8, name: "ਡਾ. ਐੱਚ. ਕੇ. ਮਨਮੋਹਨ ਸਿੰਘ", year: "(1989-1993)", },
  { image: v2, name: "ਡਾ. ਜੋਗਿੰਦਰ ਸਿੰਘ ਪੁਆਰ", year: "(1993-1999)", },
  { image: v9, name: "ਡਾ. ਜਸਬੀਰ ਸਿੰਘ ਆਹਲੂਵਾਲੀਆ", year: "(1999-2002)", },
  { image: v10, name: "ਸ. ਸਵਰਨ ਸਿੰਘ ਬੋਪਾਰਾਇ", year: "(2002-2007)", },
  { image: v7, name: "ਡਾ. ਜਸਪਾਲ ਸਿੰਘ", year: "(2007-2017)", },
  { image: "https://i1.rgstatic.net/ii/profile.image/1061691139817472-1630138301801_Q512/Bs-Ghuman.jpg", name: "ਡਾ. ਬੀ. ਐੱਸ. ਘੁੰਮਣ", year: "(2017-2020)", },
]
const settings2 = {
  dots: true,
  autoplay: true,
  autoplaySpeed: 1000000,

  slidesToShow: 3,
  slidesToScroll: 1,
};

const data = [
  {
    image: a3,
    name: "ਪ੍ਰੋ. ਹਰਬੰਸ ਸਿੰਘ",
    description:
    "ਹਰਬੰਸ ਸਿੰਘ ਇੱਕ ਸਿੱਖਿਆ ਸ਼ਾਸਤਰੀ , ਪ੍ਰਸ਼ਾਸ਼ਕ , ਵਿਦਵਾਨ ਅਤੇ ਸਿੱਖ ਧਰਮ ਦੇ ਐਨਸਾਈਕਲੋਪੀਡੀਆ ਦੇ ਮੁੱਖ ਸੰਪਾਦਕ ਸਨ। 1960 ਵਿੱਚ ਉਹ ਪਟਿਆਲਾ ਦੀ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਕਮਿਸ਼ਨ ਦੇ ਮੈਂਬਰ-ਸਕੱਤਰ ਬਣੇ। ਕਮਿਸ਼ਨ ਨੇ ਪੰਜਾਬੀ ਭਾਸ਼ਾ, ਸਾਹਿਤ ਅਤੇ ਸੱਭਿਆਚਾਰ ਨੂੰ ਅੱਗੇ ਵਧਾਉਣ ਲਈ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਬਣਾਉਣ ਲਈ ਅਹਿਮ ਭੂਮਿਕਾ ਨਿਭਾਈ ਸੀ ।ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਵਿੱਚ ਉਹਨਾਂ ਨੇ ਧਰਮ ਦੇ ਅਕਾਦਮਿਕ ਅਧਿਐਨ ਦੇ ਇੱਕ ਪੂਰੇ ਵਿਭਾਗ ਦੀ ਸਥਾਪਨਾ ਵਿੱਚ ਮਹੱਤਵਪੂਰਣ ਭੂਮਿਕਾ ਨਿਭਾਈ।",  },
  {
    image: "data:image/jpeg;base64,",
    name: "ਡਾ. ਗੰਡਾ ਸਿੰਘ",
    description:
    "ਡਾ. ਗੰਡਾ ਸਿੰਘ ਜੀ ਇੱਕ ਪੰਜਾਬੀ ਇਤਿਹਾਸਕਾਰ ਅਤੇ ਪਦਮ ਭੂਸ਼ਣ ਪੁਰਸਕਾਰ ਜੇਤੂ ਹਨ। ਉਹਨਾਂ ਨੇ ਇਤਿਹਾਸਕ ਮੁੱਲ ਦੀਆਂ ਦੋ ਦਰਜਨ ਤੋਂ ਵੱਧ ਕਿਤਾਬਾਂ ਪ੍ਰਕਾਸ਼ਿਤ ਕੀਤੀਆਂ ।ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਨੇ ਨਵੰਬਰ-1976 ਦੌਰਾਨ ਹੋਈ ਪੰਜਾਬ ਹਿਸਟਰੀ ਕਾਨਫਰੰਸ ਦੇ ਸਲਾਨਾ ਸੈਸ਼ਨ ਦੌਰਾਨ ਉਹਨਾਂ ਨੂੰ ਸਨਮਾਨਿਤ ਕੀਤਾ।ਯੂਨੀਵਰਸਿਟੀ ਨੇ ਉਸੇ ਸਾਲ ਡਾ: ਗੰਡਾ ਸਿੰਘ ਦੇ ਸਨਮਾਨ ਵਿਚ ਇਕ ਸੰਗ੍ਰਹਿ, ਐਸੇਜ਼ ਇਨ ਆਨਰ ਵੀ ਪ੍ਰਕਾਸ਼ਤ ਕੀਤਾ ਸੀ। ਯੂਨੀਵਰਸਿਟੀ ਨੇ 25 ਫਰਵਰੀ 1978 ਨੂੰ ਹੋਈ ਆਪਣੀ 15ਵੀਂ ਕਨਵੋਕੇਸ਼ਨ ਦੌਰਾਨ ਉਹਨਾਂ ਨੂੰ ਸਾਹਿਤ ਦੇ ਡਾਕਟਰ ਦੀ ਡਿਗਰੀ ਵੀ ਪ੍ਰਦਾਨ ਕੀਤੀ।",  },
   
  {
    image:"data:image/jpeg;base64,",
    name: "ਡਾ.ਜੋਧ ਸਿੰਘ ",
    description:
    "ਉਹ ਯੂਨੀਵਰਸਿਟੀ ਦੁਆਰਾ ਪ੍ਰਕਾਸ਼ਿਤ ਸਿੱਖ ਵਿਸ਼ਵਕੋਸ਼ ਦੇ ਮੁੱਖ ਸੰਪਾਦਕ ਵੀ ਸਨ। ਉਹ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਸਿੱਖ ਵਿਸ਼ਵਕੋਸ਼ ਵਿਭਾਗ ਵਿੱਚ ਪ੍ਰੋਫ਼ੈਸਰ ਵਜੋਂ ਸ਼ਾਮਲ ਹੋਏ ਸਨ। ਉਨ੍ਹਾਂ ਨੇ ਸਿੱਖ ਚਿੰਤਨ, ਦਰਸ਼ਨ ਅਤੇ ਗੁਰਬਾਣੀ 'ਤੇ ਕੰਮ ਕੀਤਾ ਸੀ। ਉਨ੍ਹਾਂ ਦੁਆਰਾ ਲਿਖੀ 'ਵਾਰਾਂ' ਦਾ ਅਨੁਵਾਦ ਵੀ ਕੀਤਾ ਸੀ। ਭਾਈ ਗੁਰਦਾਸ ਨੇ ਅੰਗਰੇਜ਼ੀ ਵਿੱਚ ਗੁਰੂ ਗ੍ਰੰਥ ਸਾਹਿਬ ਦਾ ਹਿੰਦੀ ਵਿੱਚ ਅਨੁਵਾਦ ਵੀ ਕੀਤਾ।"  },
  {
    image:"https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Surjit_Patar.jpg/640px-Surjit_Patar.jpg",
    name: "ਡਾ.ਸੁਰਜੀਤ ਪਾਤਰ",
    description:
    "ਸੁਰਜੀਤ ਪਾਤਰ ਪੰਜਾਬ, ਭਾਰਤ ਦੇ ਇੱਕ ਪੰਜਾਬੀ ਭਾਸ਼ਾ ਦੇ ਲੇਖਕ ਅਤੇ ਕਵੀ ਹਨ। ਉਹਨਾਂ ਦੀਆਂ ਕਵਿਤਾਵਾਂ ਨੇ ਆਮ ਲੋਕਾਂ ਵਿੱਚ ਬਹੁਤ ਪ੍ਰਸਿੱਧੀ ਪ੍ਰਾਪਤ ਕੀਤੀ ਹੈ ਅਤੇ ਆਲੋਚਕਾਂ ਤੋਂ ਬਹੁਤ ਪ੍ਰਸ਼ੰਸਾ ਪ੍ਰਾਪਤ ਕੀਤੀ ਹੈ। ਉਹਨਾਂ ਨੇ ਗਿਰੀਸ਼ ਦੇ ਨਾਟਕ ਨਾਗਮੰਡਲਾ, ਫੈਡਰਿਕੋ ਗਾਰਸੀਆ ਲੋਕਾਂ ਦੀਆਂ ਤਿੰਨ ਦੁਖਾਂਤਕ ਕਹਾਣੀਆਂ ਦਾ ਪੰਜਾਬੀ ਵਿੱਚ ਅਨੁਵਾਦ ਕੀਤਾ ਹੈ। ",  },
  {
    image:a1,
    name: "ਡਾ. ਦਲੀਪ ਕੌਰ ਟਿਵਾਣਾ ",
    description:
    "ਡਾ. ਦਲੀਪ ਕੌਰ ਟਿਵਾਣਾ ਸਮਕਾਲੀ ਪੰਜਾਬੀ ਸਾਹਿਤ ਦੀ ਇੱਕ ਨਾਵਲਕਾਰ ਅਤੇ ਲਘੂ-ਕਹਾਣੀ ਲੇਖਿਕਾ ਸੀ। ਉਹਨਾਂ ਨੇ ਖੇਤਰੀ ਅਤੇ ਰਾਸ਼ਟਰੀ  ਦੋਨੋਂ ਪੁਰਸਕਾਰ ਜਿੱਤੇ ਸਨ, ਅਤੇ ਇੱਕ ਵਿਆਪਕ ਅਨੁਵਾਦਿਤ ਲੇਖਕ ਸੀ। ਉਹ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਤੋਂ ਪੰਜਾਬੀ ਦੀ ਪ੍ਰੋਫੈਸਰ, ਅਤੇ ਭਾਸ਼ਾਵਾਂ ਦੀ ਡੀਨ, ਫੈਕਲਟੀ ਵਜੋਂ ਸੇਵਾਮੁਕਤ ਹੋਏ।",  },
  {
    image: a2,
    name: "ਪ੍ਰੋ. ਨਰਿੰਦਰ ਸਿੰਘ ਕਪੂਰ ",
    description:
    "ਨਰਿੰਦਰ ਸਿੰਘ ਕਪੂਰ, ਪਟਿਆਲਾ  ਪੰਜਾਬ ਦੇ ਇੱਕ ਬਹੁਤ ਮਸ਼ਹੂਰ , ਪੁਰਸਕਾਰ ਜੇਤੂ ਲੇਖਕ ਹਨ।ਉਹਨਾਂ ਦੀਆਂ ਲਿਖਤਾਂ ਸਮਾਜਿਕ, ਸੱਭਿਆਚਾਰਕ ਅਤੇ ਮਨੋਵਿਗਿਆਨਕ ਮੁੱਦਿਆਂ ਬਾਰੇ ਹਨ।1971 ਵਿਚ ਉਹ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਵਿਚ ਅੰਗਰੇਜ਼ੀ ਅਤੇ ਪੰਜਾਬੀ ਦੇ ਸਹਾਇਕ ਪ੍ਰੋਫ਼ੈਸਰ ਵਜੋਂ ਭਰਤੀ ਹੋ ਗਏ ਅਤੇ 1990 ਵਿਚ ਪੂਰਨ ਪ੍ਰੋਫ਼ੈਸਰ ਬਣ ਗਏ ਸਨ।ਇਸ ਦੌਰਾਨ ਉਹਨਾਂ ਨੇ ਆਪਣੀ ਪੜ੍ਹਾਈ ਜਾਰੀ ਰੱਖੀ ਅਤੇ ਆਪਣੀ ਯੋਗਤਾ ਵਿਚ ਹੋਰ ਵਾਧਾ ਕੀਤਾ | ਉਹਨਾਂ ਨੇ ਫਰੈਂਚ ਵਿੱਚ ਡਿਪਲੋਮਾ , ਪੱਤਰਕਾਰੀ ਵਿੱਚ ਡਿਗਰੀ (1975), ਐਲਐਲਬੀ (1978) ਅਤੇ ਪੰਜਾਬੀ ਪੱਤਰਕਾਰੀ ਵਿੱਚ ਪੀਐਚਡੀ (1978)ਵੀ ਕੀਤੀ।",  },
   {
      image:"data:image/jpeg;base64,",
      name: "ਪ੍ਰੋ. ਐਸ.ਐਸ. ਬੀਰ",
      description:"ਸਰਮੁਖ ਸਿੰਘ ਬੀਰ ਨੇ ਪੰਜਾਬ ਯੂਨੀਵਰਸਿਟੀ ਤੋਂ ਬੀਐਸਸੀ (ਆਨਰਸ ਸਕੂਲ) ਅਤੇ ਐਮਐਸਸੀ ਦੀਆਂ ਡਿਗਰੀਆਂ ਹਾਸਲ ਕੀਤੀਆਂ ਅਤੇ ਦੋਵਾਂ ਪ੍ਰੀਖਿਆਵਾਂ ਵਿੱਚ ਉਹਨਾਂ ਨੇ ਪਹਿਲਾ ਸਥਾਨ ਹਾਸਲ ਕੀਤਾ।ਉਹਨਾਂ ਨੇ 1967 ਵਿੱਚ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਵਿੱਚ ਬਨਸਪਤੀ ਵਿਭਾਗ ਦੇ ਰੀਡਰ ਅਤੇ ਮੁਖੀ ਵਜੋਂ ਕੰਮ ਕੀਤਾ । ਉਹਨਾਂ ਨੂੰ  1969 ਵਿੱਚ ਬਨਸਪਤੀ ਵਿਗਿਆਨ ਦਾ ਪ੍ਰੋਫੈਸਰ ਨਿਯੁਕਤ ਕੀਤਾ ਗਿਆ ਅਤੇ 1989 ਵਿੱਚ ਸੀਨੀਅਰ ਪ੍ਰੋਫੈਸਰ ਵਜੋਂ ਓਹ ਸੇਵਾਮੁਕਤ ਹੋਏ । ",
    },
    {
      image: "data:image/jpeg;base64,",
      name: "ਪ੍ਰੋ. ਸੰਜੀਵ ਪੁਰੀ",
      description:
      "ਪ੍ਰੋ. ਸੰਜੀਵ ਪੂਰੀ ਨੇ ਸਿੰਕ੍ਰੋਟ੍ਰੋਨ ਰੇਡੀਏਸ਼ਨ ਦੀ ਵਰਤੋਂ ਕਰਦੇ ਹੋਏ ਐਕਸ-ਰੇ ਊਰਜਾ ਖੇਤਰ ਵਿੱਚ ਫੋਟੌਨ-ਐਟਮ ਪਰਸਪਰ ਕਿਰਿਆਵਾਂ ਦੀ ਜਾਂਚ ਕੀਤੀ। ਇਹਨਾਂ ਨੇ ਈਸੀਆਰ ਸਰੋਤ ਦੀ ਵਰਤੋਂ ਕਰਦੇ ਹੋਏ ਘੱਟ ਵੇਗ ਵਾਲੇ ਪ੍ਰੋਜੈਕਟਾਈਲ ਪ੍ਰਭਾਵ ਲਈ ਆਇਨ-ਐਟਮ ਇੰਟਰੈਕਸ਼ਨ ਪ੍ਰਕਿਰਿਆਵਾਂ ਦੀ ਜਾਂਚ ਵੀ ਕੀਤੀ ।",    },
      {
        image: "https://html.scribdassets.com/6porpyvq9s94ql6t/images/1-a0de5e0429.png",
        name: "ਪ੍ਰੋ. ਬਲਦੇਵ ਸਿੰਘ ਸੂਦ",
        description:
        "ਪ੍ਰੋ: ਬੀ.ਐਸ. ਸੂਦ ਨੇ 25 ਸਾਲ (1964-89) ਤੱਕ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਪਟਿਆਲਾ ਦੀ ਸੇਵਾ ਕੀਤੀ। ਆਪਣੀ ਸੇਵਾਮੁਕਤੀ ਤੋਂ ਬਾਅਦ, ਉਹ ਹੋਰ ਸੱਤ ਸਾਲਾਂ (1989-96) ਲਈ ਡਾਇਰੈਕਟਰ ਨਿਊਕਲੀਅਰਸਾਇੰਸ ਲੈਬਾਰਟਰੀਆਂ ਵਜੋਂ ਸੇਵਾ ਕਰਦਾ ਰਹੇ । ਪੀ.ਯੂ. ਪਟਿਆਲਾ ਵਿੱਚ, ਉਹਨਾਂ ਨੇ ਇਹਨਾਂ ਤਕਨੀਕਾਂ ਨੂੰ ਹੋਰ ਵਿਕਸਤ ਕੀਤਾ ਅਤੇ ਆਪਣੇ ਵਿਦਿਆਰਥੀਆਂ ਨੂੰ ਸਿਧਾਂਤਕ ਪੂਰਵ-ਅਨੁਮਾਨਾਂ ਨਾਲ ਪ੍ਰਾਪਤ ਕੀਤੇ ਪ੍ਰਯੋਗਾਤਮਕ ਨਤੀਜਿਆਂ ਦੀ ਤੁਲਨਾ ਕਰਨ ਲਈ ਮਾਰਗਦਰਸ਼ਨ ਕੀਤਾ।" },    
    
];

const HomePage = () => {
 

  const educationDropdownArrow = useRef(null);
  const educationList = useRef(null);

  const secondArrow = useRef(null);
  const secondList = useRef(null);

  const thirdArrow = useRef(null);
  const thirdList = useRef(null);

  const fourthArrow = useRef(null);
  const fourthList = useRef(null);

  const fifthArrow = useRef(null);
  const fifthList = useRef(null);

  function filterDropdownHandler(dropdownArrow, list) {
    list.current.classList.toggle("hideDrop");
    list.current.classList.toggle("pt-2");
    list.current.classList.toggle("hidden");
    dropdownArrow.current.classList.toggle("rotate-180");
  }


  return (
    <>
   

      <h1 className="font-bold text-2xl ml-10 p-6">
      ਯੂਨੀਵਰਸਿਟੀ ਬਾਰੇ
      </h1>
      <div className=" flex justify-end -mt-16 px-16">
            <h1 className="font-bold text-2xl px-4 py-2"> ਯੂਨੀਵਰਸਿਟੀ ਧੂਨੀ </h1>
            <iframe src={pup} alt="campus" className="rounded-lg h-12 w-96" /><br></br>
          </div>
      <div className="flex mx-5">
        <div className="w-[60vw] mr-5 ml-10">
          <p className="p-2  text-justify space-x-4">
          ਪੰਜਾਬ ਵਿਧਾਨ ਸਭਾ ਨੇ 1961 ਦੇ ਪੰਜਾਬ ਐਕਟ ਨੰ. 35 ਅਧੀਨ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਪਟਿਆਲਾ ਦੀ ਸਥਾਪਨਾ 30 ਅਪ੍ਰੈਲ 1962 ਨੂੰ ਕੀਤੀ ਗਈ। ਇਸ ਉਪਰੰਤ ਭਾਰਤ ਦੇ ਰਾਸ਼ਟਰਪਤੀ ਡਾ. ਐਸ. ਰਾਧਾ. ਕ੍ਰਿਸ਼ਨਨ ਵਲੋਂ 24 ਜੂਨ 1962 ਈ. ਵਿਚ ਇਸਦਾ ਨੀਂਹ ਪੱਥਰ ਰੱਖਿਆ ਗਿਆ। ਉਹਨਾਂ ਆਪਣੇ ਭਾਸ਼ਣ ਵਿਚ ਕਿਹਾ ਕਿ ਉੱਚ ਸਿੱਖਿਆ ਦੇਣ ਵਾਲੀਆ ਸੰਸਥਾਵਾਂ ਦੇਸ਼/ਕੌਮ ਦੀ ਉਸਾਰੀ ਵਿਚ ਅਹਿਮ ਯੋਗਦਾਨ ਨਿਭਾਉਂਦੀਆਂ ਹਨ। ਸਾਡਾ ਟੀਚਾ ਲੋਕਤੰਤਰ ਭਾਰਤ ਵਿਚ ਸ਼ਕਤੀਸ਼ਾਲੀ ਅਤੇ ਆਜ਼ਾਦ ਨਾਗਰਿਕ ਬਣਾਉਣਾ ਹੈ ਜਿਹਨਾਂ ਵਿਚ ਵਧਣ-ਫੁੱਲਣ ਦੀ ਬਰਾਬਰ ਸੰਭਾਵਨਾ ਹੋਵੇ। ਇਸ ਕੰਮ ਲਈ ਅਹਿਮ ਜ਼ਿੰਮੇਵਾਰੀ ਯੂਨੀਵਰਸਿਟੀਆਂ ਦੀ ਹੁੰਦੀ ਹੈ। ਇਜ਼ਰਾਈਲ ਦੀ ਹਿਬਰੋ ਯੂਨੀਵਰਸਿਟੀ ਤੋਂ ਬਾਅਦ ਸ਼ਾਹੀ ਰਿਆਸਤ ਪਟਿਆਲਾ ਵਿਚ ਸਥਾਪਤ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਦੁਨੀਆਂ ਦੀ ਅਜਿਹੀ ਦੂਜੀ ਯੂਨੀਵਰਸਿਟੀ ਹੈ, ਜੋ ਭਾਸ਼ਾ ਦੇ ਨਾਮ ‘ਤੇ ਸਥਾਪਿਤ ਕੀਤੀ ਗਈ ਹੈ। ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਦਾ ਮੁੱਖ ਮੰਤਵ, ਪੰਜਾਬੀ ਭਾਸ਼ਾ, ਸਾਹਿਤ ਅਤੇ ਸਭਿਆਚਾਰ ਦਾ ਪ੍ਰਚਾਰ-ਪ੍ਰਸਾਰ ਕਰਨਾ ਹੈ। ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ ਕੇਵਲ ਪੰਜਾਬੀ ਭਾਸ਼ਾ ਨੂੰ ਲੈ ਕੇ ਹੀ ਕਾਰਜਸ਼ੀਲ ਨਹੀਂ ਬਲਕਿ ਇਸ ਵਿਚ ਵੱਖ-ਵੱਖ ਗਿਆਨ ਖੇਤਰਾਂ ਨਾਲ ਸਬੰਧਿਤ 70 ਦੇ ਲਗਭਗ ਵਿਭਾਗ ਹਨ, ਜਿਨ੍ਹਾਂ ਵਿੱਚ ਜਿੱਥੇ ਸਾਇੰਸ, ਇੰਜੀਨੀਅਰਿੰਗ, ਸੋਸ਼ਲ ਸਾਇੰਸਜ, ਧਰਮ, ਅਰਥ ਸ਼ਾਸਤਰ, ਮਨੈਜਮੈਂਟ ਆਦਿ ਖੇਤਰਾਂ ਨਾਲ ਸਬੰਧਿਤ ਅਧਿਆਪਨ ਕਾਰਜ ਕਰਵਾਇਆ ਜਾਂਦਾ ਹੈ, ਉਥੇ ਇਨ੍ਹਾਂ ਖੇਤਰਾਂ ਨਾਲ ਜੁੜੇ ਵਿਸ਼ਿਆਂ ਵਿਚ ਐੱਮ.ਫਿਲ ਅਤੇ ਪੀਐੱਚ. ਡੀ ਪੱਧਰ ਦਾ ਖੋਜ ਕਾਰਜ ਵੀ ਕਰਵਾਇਆ ਜਾਂਦਾ ਹੈ। ਇਸ ਯੂਨੀਵਰਸਿਟੀ ਨਾਲ ਸਬੰਧਿਤ ਵਿਦਿਆਰਥੀਆਂ ਨੇ ਵੱਖ-ਵੱਖ ਖੇਤਰਾਂ ਵਿਚ ਵੱਡੀਆਂ ਮੱਲਾਂ ਮਾਰੀਆਂ ਹਨ। ਜਿਵੇਂ ਫਿਲਮ ਅਤੇ ਸੰਗੀਤ ਜਗਤ ਨਾਲ ਸਬੰਧਿਤ ਵੱਡੇ ਚਿਹਰੇ ਇਸ ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਪੈਦਾ ਕੀਤੇ ਹਨ। ਇਸਦੇ ਨਾਲ ਜੁੜੇ ਵਿਦਵਾਨ ਤੇ ਆਲੋਚਕ ਅੰਤਰ-ਰਾਸ਼ਟਰੀ ਪ੍ਰਸਿੱਧੀ ਰੱਖਦੇ ਹੋਏ ਵਿਸ਼ਵ ਪੱਧਰ ਦੀਆਂ ਕਾਨਫ਼ਰੰਸਾਂ ਵਿਚ ਆਪਣੇ ਮੁੱਲਵਾਨ ਵਿਚਾਰ ਪੇਸ਼ ਕਰਦੇ ਰਹੇ ਹਨ। ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ 316 ਏਕੜ ਦੇ ਹਰੇ-ਭਰੇ ਪ੍ਰਦੂਸ਼ਣ ਮੁਕਤ ਕੈਂਪਸ ਵਿੱਚ 1965 ਤੋਂ ਵਿਦਿਆ ਦਾ ਚਾਨਣ ਵੰਡ ਰਹੀ ਹੈ।
            </p>
            {/* <img
            src={main_img}
            alt=""
            className="w-96 justify-center px-auto"
          /> */}
          <p className=" text-justify">
            ਸ਼ੁਰੂਆਤ ਵਿੱਚ ਯੂਨੀਵਰਸਿਟੀ ਦੇ ਅਧੀਨ 16 ਕਿਲੋਮੀਟਰ ਦੇ ਖੇਤਰ ਵਿੱਚ ਪੈਂਦੇ 9 ਕਾਲਜਾਂ ਨੂੰ ਇਸ ਨਾਲ ਜੋੜਿਆ ਗਿਆ ਅਤੇ ਬਾਅਦ ਵਿੱਚ ਇਹ ਗਿਣਤੀ 43 ਹੋ ਗਈ। ਅੱਜ ਇਹ ਯੂਨੀਵਰਸਿਟੀ ਪੰਜਾਬ ਦੇ ਨੌਂ ਜ਼ਿਲ੍ਹਿਆਂ ਦੀਆਂ ਵਿੱਦਿਅਕ ਲੋੜਾਂ ਸੰਪੂਰਨ ਸਮਰੱਥਾ ਨਾਲ ਪੂਰੀਆਂ ਕਰ ਰਹੀ ਹੈ। ਆਰੰਭ ਤੋਂ ਲੈ ਕੇ ਸਮੇਂ ਦੇ ਨਾਲ-ਨਾਲ ਸਰਵ-ਪੱਖੀ ਅਤੇ ਵਿੱਦਿਆ ਲਈ ਸਥਾਪਤ ਇਹ ਵਿੱਦਿਅਕ ਸੰਸਥਾ, ਉਚੇਰੀ ਵਿੱਦਿਆ ਦੇ ਪਾਸਾਰ, ਮਾਨਵਤਾ ਭਲਾਈ, ਹੁਨਰਾਂ, ਵਿਗਿਆਨਾਂ, ਇੰਜੀਨੀਅਰਿੰਗ ਭਾਸ਼ਾਵਾਂ, ਤਕਨਾਲੋਜੀ ਅਤੇ ਹੋਰ ਬਹੁਤ ਸਾਰੇ ਵਿਸ਼ਿਆਂ ਦੇ ਮਿਆਰੀ ਖੋਜ ਕਾਰਜਾਂ ਲਈ ਇੱਕ ਵਰਦਾਨ ਸਿੱਧ ਹੋਈ ਹੈ। 600 ਏਕੜ ਜ਼ਮੀਨ ਤੇ ਫੈਲੀ ਹੋਈ ਇਹ ਯੂਨੀਵਰਸਿਟੀ ਜਿਸ ਵਿਚ 1500+ ਦੇ ਲਗਭਗ ਯੋਗ ਅਧਿਆਪਕ 14,000+ ਵਿਦਿਆਰਥੀਆਂ ਨੂੰ ਬਹੁ-ਪੱਖੀ, ਬਹੁ-ਖੋਜੀ ਅਤੇ ਬਹੁ-ਪ੍ਰਤਿਭੀ ਸਿੱਖਿਆ ਬਹੁਤ ਹੀ ਢੁਕਵੇਂ ਵਾਤਾਵਰਨ ਵਿੱਚ ਪਰਦਾਨ ਕਰ ਰਹੇ ਹਨ, ਕੈਂਪਸ ਵਿੱਚ 70+ ਸਿੱਖਿਆ ਅਤੇ ਖੋਜ ਵਿਭਾਗ/ਚੇਅਰਾਂ ਹਨ, ਇਸ ਨਾਲ 27 ਖੇਤਰੀ ਕੇਂਦਰ/ ਨੇਬਰਹੁੱਡ ਕੈਂਪਸ/ਕਾਨਸਟੀਚੀਊਐਂਟ ਕਾਲਜ ਅਤੇ 274 ਕਾਲਜ ਜੁੜੇ ਹੋਏ ਹਨ।
          </p>
          
         

          <hr />
        </div>
        <div className=" p-5">
                  <iframe className="aspect-video  h-full w-[600px] rounded-lg"
src="https://www.youtube.com/embed/WYRf33XU2os?playlist=WYRf33XU2os&loop=1"/>

          {/* <iframe className="aspect-video  h-full w-[600px] rounded-lg"
            src={`${promotion}`} allow="autoplay"> frameborder="0" allow="accelerometer; ; clipboard-write; encrypted-media; gyroscope; picture-in-picture"</iframe> */}
        </div>
      </div>

      <div className=" flex flex-col w-full px-12">
        <div className="flex justify-center pb-10 w-full overflow-hidden">

          <div className="max-w-xxl px-8 py-4 mx-auto mt-16 bg-white rounded-lg border-2 border-slate-300 bg-slate-100 shadow-lg">
            <div className="flex justify-center -mt-20 md:justify-end">
              <img className="object-cover w-36 h-36 border-4 border-blue-500 rounded-full dark:border-blue-400" alt=""
                src={VC}
              />
            </div>

            <h2 className="text-2xl font-semibold text-gray-800  md:mt-0 md:text-2xl">ਉਪ-ਕੁਲਪਤੀ ਵੱਲੋਂ ਸੰਦੇਸ਼</h2>

            <p className="mt-2 text-gray-600  text-justify">
            ਉਚੇਰੀ ਸਿੱਖਿਆ ਦੀਆਂ ਸਿਖਰਲੀਆਂ ਸੰਸਥਾਵਾਂ ਵਿਚੋਂ ਇੱਕ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਦੇ ਵਾਈਸ ਚਾਂਸਲਰ ਦੇ ਤੌਰ ‘ਤੇ ਸੇਵਾ ਨਿਭਾਉਣ ਦਾ ਮੌਕਾ ਮਿਲਣ ਉੱਤੇ ਮੈਂ ਮਾਣਮੱਤਾ ਮਹਿਸੂਸ ਕਰਦਾ ਹਾਂ। ਇਹ ਯੂਨੀਵਰਸਿਟੀ 30 ਅਪ੍ਰੈਲ 1962 ਨੂੰ ਪੰਜਾਬੀ ਭਾਸ਼ਾ, ਸਾਹਿਤ ਅਤੇ ਸਭਿਆਚਾਰ ਦੇ ਪ੍ਰਚਾਰ-ਪ੍ਰਸਾਰ ਲਈ ਅਤੇ ਪੰਜਾਬੀ ਨੂੰ ਵਿਸ਼ਵ ਭਰ ਦੇ ਗਿਆਨ-ਸੰਚਾਰ ਅਤੇ ਉਤਪਾਦਨ ਦਾ ਮਾਧਿਅਮ ਬਣਾਉਣ ਦੇ ਮਕਸਦ ਨਾਲ ਸਥਾਪਿਤ ਕੀਤੀ ਗਈ ਸੀ | ਭਾਸ਼ਾ ਦੇ ਨਾਂ ਉੱਤੇ ਬਣਨ ਵਾਲੀ ਇਹ ਵਿਸ਼ਵ ਦੀ ਦੂਜੀ ਯੂਨੀਵਰਸਿਟੀ ਸੀ। ਹੁਣ ਇਹ ਯੂਨੀਵਰਸਿਟੀ ਜੀਵ ਵਿਗਿਆਨਾਂ, ਭੌਤਿਕ ਵਿਗਿਆਨਾਂ, ਮੈਡੀਸਨ, ਇੰਜਨੀਅਰਿੰਗ, ਤਕਨਾਲੋਜੀ, ਬਿਜ਼ਨੱਸ-ਅਧਿਐਨ, ਕਾਨੂੰਨ, ਸਮਾਜਕ ਵਿਗਿਆਨਾਂ, ਭਾਸ਼ਾਵਾਂ, ਸਿੱਖਿਆ, ਸੂਚਨਾ-ਵਿਗਿਆਨਾਂ, ਕਲਾ ਅਤੇ ਸਭਿਆਚਾਰ ਦੇ ਖੇਤਰਾਂ ਵਿਚ ਉੱਚ ਪੱਧਰੀ ਸਿੱਖਿਆ ਪ੍ਰਦਾਨ ਕਰਨ ਅਤੇ ਨਵੀਨਤਮ ਖੋਜਾਂ ਕਰਨ ਵਾਲੀ ਮੁਖ ਸੰਸਥਾ ਦੇ ਤੌਰ ‘ਤੇ ਸਾਹਮਣੇ ਆਈ ਹੈ। ਇਹ ਵੱਡੇ ਮਾਣ ਵਾਲੀ ਗੱਲ ਹੈ ਕਿ ਨੈਸ਼ਨਲ ਅਸੈੱਸਮੈਂਟ ਐਂਡ ਐਕਰੀਡੀਸ਼ਨ ਕਾਉਂਸਿਲ (ਐੱਨ.ਏ.ਏ.ਸੀ.) ਨੇ ਯੂਨੀਵਰਸਿਟੀ ਦੀ ਸਮੁੱਚੀ ਕਾਰਗ਼ੁਜ਼ਾਰੀ ਦਾ ਮੁਲਾਂਕਣ ਕਰਦਿਆਂ ਪਹਿਲੇ ਗੇੜ (2002-07) ਵਿਚ ਪੰਜ ਸਟਾਰ ਗਰੇਡ ਪ੍ਰਦਾਨ ਕੀਤਾ ਸੀ, ਦੂਜੇ ਗੇੜ (2008-13) ਅਤੇ ਤੀਜੇ ਗੇੜ (2016-23) ਵਿਚ ‘ਏ’ ਗਰੇਡ ਪ੍ਰਦਾਨ ਕੀਤਾ ਸੀ। ਇਸ ਸਮੇਂ ਯੂਨੀਵਰਸਿਟੀ ਵਿਚ 65 ਅਧਿਆਪਨ ਅਤੇ ਖੋਜ ਵਿਭਾਗ, 275 ਸੰਬੰਧਤਾ ਪ੍ਰਾਪਤ ਕਾਲਜ, 5 ਨੇਬਰਹੁੱਡ ਕੈਂਪਸ, 14 ਕਾਂਸਟੀਚੁਐਂਟ ਕਾਲਜ ਅਤੇ 4 ਰੀਜਨਲ ਸੈਂਟਰ ਹਨ। 
            </p>
            <div className=" flex justify-end">
              <button
                type="submit"
                className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
             ਹੋਰ ਜਾਣੋ
              </button>
            </div>


            <div className="flex justify-end mt-4">
              <h3 class="text-xl hover:underline decoration-wavy  font-medium text-blue-500 ">ਪ੍ਰੋਫੈਸਰ ਅਰਵਿੰਦ</h3>
            </div>
          </div>


          <div className="w-full h-96 mt-20 mx-2 px-3">
          <iframe className="aspect-video  h-full w-[350px] rounded-lg"
src="https://www.youtube.com/embed/WYRf33XU2os?playlist=WYRf33XU2os&loop=1"/>
            {/* <iframe src={campus} alt="campus" className="h-full  rounded-lg" /> */}
          </div>
        </div>

        <div className="grid  gap-4 grid-cols-2 pb-10">

          <div className="max-w-xxl px-8 py-4 mx-auto mt-16 bg-white rounded-lg border-2 border-slate-300  bg-slate-100 shadow-lg ">
            <div className="flex justify-center -mt-16 md:justify-end">
              <img className="object-cover w-28 h-28 border-4 border-blue-500 rounded-full dark:border-blue-400" alt="Testimonial avatar"
                src="data:image/jpeg;base64,"
              />
            </div>

            <h2 className="text-2xl font-semibold text-gray-800  md:mt-0 md:text-2xl"> ਸੰਦੇਸ਼ : ਡੀਨ ਅਲੂਮਨੀ</h2>

            <p className="mt-2 text-gray-600  text-justify ">ਉਚੇਰੀ ਸਿੱਖਿਆ ਦੀਆਂ ਸਿਖਰਲੀਆਂ ਸੰਸਥਾਵਾਂ ਵਿਚੋਂ ਇੱਕ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਦੇ ਵਾਈਸ ਚਾਂਸਲਰ ਦੇ ਤੌਰ ‘ਤੇ ਸੇਵਾ ਨਿਭਾਉਣ ਦਾ ਮੌਕਾ ਮਿਲਣ ਉੱਤੇ ਮੈਂ ਮਾਣਮੱਤਾ ਮਹਿਸੂਸ ਕਰਦਾ ਹਾਂ। ਇਹ ਯੂਨੀਵਰਸਿਟੀ 30 ਅਪ੍ਰੈਲ 1962 ਨੂੰ ਪੰਜਾਬੀ ਭਾਸ਼ਾ, ਸਾਹਿਤ ਅਤੇ ਸਭਿਆਚਾਰ ਦੇ ਪ੍ਰਚਾਰ-ਪ੍ਰਸਾਰ ਲਈ ਅਤੇ ਪੰਜਾਬੀ ਨੂੰ ਵਿਸ਼ਵ ਭਰ ਦੇ ਗਿਆਨ-ਸੰਚਾਰ ਅਤੇ ਉਤਪਾਦਨ ਦਾ ਮਾਧਿਅਮ ਬਣਾਉਣ ਦੇ ਮਕਸਦ ਨਾਲ ਸਥਾਪਿਤ ਕੀਤੀ ਗਈ ਸੀ | ਭਾਸ਼ਾ ਦੇ ਨਾਂ ਉੱਤੇ ਬਣਨ ਵਾਲੀ ਇਹ ਵਿਸ਼ਵ ਦੀ ਦੂਜੀ ਯੂਨੀਵਰਸਿਟੀ ਸੀ।</p>
            <div className="flex justify-end">
              <button
                type="submit"
                className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
             ਹੋਰ ਜਾਣੋ
              </button>
            </div>


            <div className="flex justify-end mt-4">
              <h3 class="text-xl hover:underline decoration-wavy  font-medium text-blue-500 ">ਪ੍ਰੋ.ਸਤਨਾਮ ਸਿੰਘ ਸੰਧੂ</h3>
            </div>
          </div>


          <div className="max-w-xxl px-8 py-4 mx-auto mt-16 bg-white rounded-lg  border-2 border-slate-300  bg-slate-100 shadow-lg ">
            <div className="flex justify-center -mt-16 md:justify-end">
              <img className="object-cover w-28 h-28 border-4 border-blue-500 rounded-full dark:border-blue-400" alt=""
                src="https://csepup.ac.in/images/dws.png"

              />
            </div>

            <h2 className="text-2xl font-semibold text-gray-800  md:mt-0 md:text-2xl">ਸੰਦੇਸ਼ :  ਕੋਆਰਡੀਨੇਟਰ </h2>

            <p className="mt-2 text-gray-600  text-justify">ਉਚੇਰੀ ਸਿੱਖਿਆ ਦੀਆਂ ਸਿਖਰਲੀਆਂ ਸੰਸਥਾਵਾਂ ਵਿਚੋਂ ਇੱਕ ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਦੇ ਵਾਈਸ ਚਾਂਸਲਰ ਦੇ ਤੌਰ ‘ਤੇ ਸੇਵਾ ਨਿਭਾਉਣ ਦਾ ਮੌਕਾ ਮਿਲਣ ਉੱਤੇ ਮੈਂ ਮਾਣਮੱਤਾ ਮਹਿਸੂਸ ਕਰਦਾ ਹਾਂ। ਇਹ ਯੂਨੀਵਰਸਿਟੀ 30 ਅਪ੍ਰੈਲ 1962 ਨੂੰ ਪੰਜਾਬੀ ਭਾਸ਼ਾ, ਸਾਹਿਤ ਅਤੇ ਸਭਿਆਚਾਰ ਦੇ ਪ੍ਰਚਾਰ-ਪ੍ਰਸਾਰ ਲਈ ਅਤੇ ਪੰਜਾਬੀ ਨੂੰ ਵਿਸ਼ਵ ਭਰ ਦੇ ਗਿਆਨ-ਸੰਚਾਰ ਅਤੇ ਉਤਪਾਦਨ ਦਾ ਮਾਧਿਅਮ ਬਣਾਉਣ ਦੇ ਮਕਸਦ ਨਾਲ ਸਥਾਪਿਤ ਕੀਤੀ ਗਈ ਸੀ | ਭਾਸ਼ਾ ਦੇ ਨਾਂ ਉੱਤੇ ਬਣਨ ਵਾਲੀ ਇਹ ਵਿਸ਼ਵ ਦੀ ਦੂਜੀ ਯੂਨੀਵਰਸਿਟੀ ਸੀ। </p>
            <div className=" flex justify-end">
              <button
                type="submit"
                className=" py-2 px-4 text-sm font-medium rounded-md text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
             ਹੋਰ ਜਾਣੋ
              </button>
            </div>


            <div className="flex justify-end mt-4">
              <h3 class="text-xl hover:underline decoration-wavy  font-medium text-blue-500 ">ਪ੍ਰੋ. ਵਿਲੀਅਮ ਜੀਤ ਸਿੰਘ</h3>
            </div>
          </div>


        </div>
      </div>


  
      <div className=" my-8  grid grid-cols-2" >
        
      <div className="grid grid-cols-5 px-5 pt-12  gap-1 hover:cursor-pointer">
        
   <img
            src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(16).JPG"
            alt=""
            className="hover:scale-[2] rounded-lg"
          />
   <img
            src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(17).JPG"
            alt=""
            className="hover:scale-[2] rounded-lg"
          /> <img
          src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(15).JPG"
          alt=""
          className="hover:scale-[2] rounded-lg"
        /> <img
        src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(14).JPG"
        alt=""
        className="hover:scale-[2] rounded-lg"
      /> <img
      src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(24).JPG"
      alt=""
      className="hover:scale-[2] rounded-lg"
    /> <img
    src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(22).JPG"
    alt=""
    className="hover:scale-[2] rounded-lg"
  /> <img
  src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Functions/1%20(42).JPG"
  alt=""
  className="hover:scale-[2] rounded-lg"
/>
 <img
            src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(8).JPG"
            alt=""
            className="hover:scale-[2] rounded-lg"
          /> <img
          src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(13).JPG"
          alt=""
          className="hover:scale-[2] rounded-lg"
        /> <img
        src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(13).JPG"
        alt=""
        className="hover:scale-[2] rounded-lg"
      /> <img
      src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(20).JPG"
      alt=""
      className="hover:scale-[2] rounded-lg"
    /> 
    <img
      src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(37).JPG"
      alt=""
      className="hover:scale-[2] rounded-lg"
    /> 
    <img
      src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(40).JPG"
      alt=""
      className="hover:scale-[2] rounded-lg"
    /><img
    src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(44).JPG"
    alt=""
    className="hover:scale-[2] rounded-lg"
  /><img
  src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(68).JPG"
  alt=""
  className="hover:scale-[2] rounded-lg"
/><img
      src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(77).JPG"
      alt=""
      className="hover:scale-[2] rounded-lg"
    /><img
    src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(81).JPG"
    alt=""
    className="hover:scale-[2] rounded-lg"
  /><img
  src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(84).JPG"
  alt=""
  className="hover:scale-[2] rounded-lg"
/><img
      src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(85).JPG"
      alt=""
      className="hover:scale-[2] rounded-lg"
    /><img
    src="http://www.punjabiuniversity.ac.in/pages/Images/Alumni/Dept/1%20(88).JPG"
    alt=""
    className="hover:scale-[2] rounded-lg"
  />

    
    
    
    
     </div>
    
        <div >
          <h1 className="font-bold text-3xl p-1   text-center" >
          ਮੰਤਵ ਅਤੇ ਉਦੇਸ਼
          </h1>
          <dl className="mt-8 grid grid-cols-2  sm:gap-y-8 lg:gap-x-6">
            {features.map((feature) => (
              <div key={feature.description} className="border-t border-gray-200 pt-4">
                <dd className=" text-sm text-gray-900  text-justify px-4">{feature.description}</dd>
              </div>
            ))}
          </dl>

        </div>

      </div>
      <h1 className=" text-3xl text-black-900 text-center font-bold  my-4 py-5">
      ਅੰਤਰਰਾਸ਼ਟਰੀ ਅਲੂਮਨੀ ਚੈਪਟਰ
      </h1>

      <div className="grid  grid-cols-2 gap-1">
        <div className="w-full relative">
          <h1 className='text-7xl text-center absolute bg-red-600  mt-40 text-white w-full'>ਅਮਰੀਕਾ</h1>
          <img
            src="https://www.daad-canada.ca/files/2016/04/Web_Group_5183-624x351.jpg"
            alt=""
            className="w-full"
          />
        </div>
        <div className=" w-full relative">
          <h1 className='text-7xl text-center absolute bg-red-600  mt-40 text-white w-full'>ਕਨੇਡਾ</h1>
          <img
            src="https://upwithpeople.s3.us-west-2.amazonaws.com/wp-content/uploads/2018/11/23144341/Victoria-1024x768.jpg"
            alt=""
            className="w-full h-[400px]"
          />
        </div>
      </div>


      {/* Accordion Code here */}

      <h1 className=" text-3xl text-black-900 text-center font-bold  mt-4 pt-5">
       
ਅਹੁਦੇਦਾਰਾਂ ਦੀਆਂ ਡਿਊਟੀਆਂ ਅਤੇ ਕੰਮ
      </h1>

      <div className="acc-body">
        <div className="flex font-bold justify-between items-center acc " onClick={() => filterDropdownHandler(educationDropdownArrow, educationList)}>
          <h1 className="font-bold mb-1 text-lg ">ਮੁੱਖ ਸਰਪ੍ਰਸਤ</h1>
          <div ref={educationDropdownArrow} className="arrow">
            <BiChevronDown className="h-6 w-6" />
          </div>
        </div>
        <div className="h-0 hidden acc-child text-gray-900 text-lg font-medium my-2" ref={educationList}>
          <ol className=" list-inside list-decimal text-left space-x-4  text-justify">
            <li className="ml-4">ਚੀਫ ਸਰਪ੍ਰਸਤ ਐਸੋਸੀਏਸ਼ਨ ਦੀਆਂ ਮੀਟਿੰਗਾਂ, ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ ਦੀਆਂ ਮੀਟਿੰਗਾਂ ਅਤੇ ਹੋਰ ਸਾਰੇ ਕਾਰਜਾਂ ਦੀ ਪ੍ਰਧਾਨਗੀ ਕਰੇਗਾ। ਜੋ ਕਿ ਐਸੋਸੀਏਸ਼ਨ ਦੁਆਰਾ ਆਯੋਜਿਤ ਕੀਤਾ ਜਾ ਸਕਦਾ ਹੈ।</li>
            <li>ਚੀਫ ਸਰਪ੍ਰਸਤ ਐਸੋਸੀਏਸ਼ਨ ਦੀ ਜਾਇਦਾਦ ਅਤੇ ਹਿੱਤਾਂ ਦਾ ਰਖਵਾਲਾ ਹੋਵੇਗਾ.</li>
            <li>ਉਹ ਐਸੋਸੀਏਸ਼ਨ ਦੀ ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ/ਜਨਰਲ ਬਾਡੀ ਦੀਆਂ ਵਿਸ਼ੇਸ਼ ਮੀਟਿੰਗਾਂ ਬੁਲਾਵੇਗਾ ਜੇਕਰ ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ ਦੇ ਮਾਮਲੇ ਵਿੱਚ ਘੱਟੋ-ਘੱਟ 7 ਮੈਂਬਰਾਂ ਅਤੇ ਐਸੋਸੀਏਸ਼ਨ ਦੇ ਮਾਮਲੇ ਵਿੱਚ 30 ਮੈਂਬਰਾਂ ਵੱਲੋਂ ਦਸਤਖਤ ਕੀਤੀ ਲਿਖਤੀ ਮੰਗ ਉਸ ਨੂੰ ਪੇਸ਼ ਕੀਤੀ ਜਾਂਦੀ ਹੈ।</li>
          </ol>
        </div>

        <div className="flex font-bold justify-between items-center acc my-2" onClick={() => filterDropdownHandler(secondArrow, secondList)}>
          <h1 className="font-bold mb-1 text-lg">ਪ੍ਰਧਾਨ / ਸੀਨੀਅਰ ਮੀਤ ਪ੍ਰਧਾਨ</h1>
          <div ref={secondArrow} className="arrow">
            <BiChevronDown className="h-6 w-6" />
          </div>
        </div>
        <div className="h-0 hidden acc-child text-gray-900 text-lg font-medium my-2" ref={secondList}>
          <ol className=" list-inside list-decimal text-left space-x-4  text-justify">
            <li className="ml-4">ਪ੍ਰਧਾਨ ਆਪਣੀ ਗੈਰ-ਹਾਜ਼ਰੀ ਵਿੱਚ ਮੁੱਖ ਸਰਪ੍ਰਸਤ ਦੇ ਕਰਤੱਵਾਂ ਅਤੇ ਕਾਰਜ ਕਰੇਗਾ। ਸੀਨੀਅਰ ਮੀਤ-ਪ੍ਰਧਾਨ ਆਪਣੀ ਗੈਰ-ਹਾਜ਼ਰੀ ਵਿੱਚ ਪ੍ਰਧਾਨ ਦੇ ਕਰਤੱਵਾਂ ਅਤੇ ਕੰਮ ਕਰੇਗਾ।</li>
            <li>ਪ੍ਰਧਾਨ/ਸੀਨੀਅਰ ਉਪ-ਪ੍ਰਧਾਨ ਅਜਿਹੇ ਹੋਰ ਕਰਤੱਵਾਂ ਅਤੇ ਕਾਰਜਾਂ ਨੂੰ ਨਿਭਾਉਣਗੇ ਜੋ ਮੁੱਖ ਸਰਪ੍ਰਸਤ ਜਾਂ ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ ਦੁਆਰਾ ਉਸ ਨੂੰ ਸੌਂਪੇ ਜਾ ਸਕਦੇ ਹਨ।</li>
          </ol>
        </div>

        <div className="flex font-bold justify-between items-center acc my-2" onClick={() => filterDropdownHandler(thirdArrow, thirdList)}>
          <h1 className="font-bold mb-1 text-lg">ਜਨਰਲ ਸਕੱਤਰ</h1>
          <div ref={thirdArrow} className="arrow">
            <BiChevronDown className="h-6 w-6" />
          </div>
        </div>
        <div className="h-0 hidden acc-child  text-gray-900 text-lg font-medium my-2" ref={thirdList}>
          <ol className=" list-inside list-decimal text-left space-x-4  text-justify">
            <li className="ml-4">ਉਹ ਐਸੋਸੀਏਸ਼ਨ ਦੇ ਸਾਰੇ ਰਿਕਾਰਡਾਂ ਅਤੇ ਦਸਤਾਵੇਜ਼ਾਂ ਦਾ ਰਖਵਾਲਾ ਹੋਵੇਗਾ।</li>
            <li>ਉਹ ਐਸੋਸੀਏਸ਼ਨ ਦੇ ਫੰਡਾਂ ਦੀ ਸੁਰੱਖਿਅਤ ਹਿਰਾਸਤ ਲਈ ਜ਼ਿੰਮੇਵਾਰ ਹੋਵੇਗਾ ਅਤੇ ਐਸੋਸੀਏਸ਼ਨ ਦੀ ਤਰਫੋਂ ਸਕੱਤਰ ਜਾਂ ਸੰਯੁਕਤ-ਸਕੱਤਰ-ਕਮ-ਖਜ਼ਾਨਚੀ ਨਾਲ ਸਾਂਝੇ ਤੌਰ 'ਤੇ ਬੈਂਕ ਖਾਤਿਆਂ ਦਾ ਸੰਚਾਲਨ ਕਰੇਗਾ।</li>
            <li>ਉਹ ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ ਅਤੇ ਐਸੋਸੀਏਸ਼ਨ ਦੀਆਂ ਮੀਟਿੰਗਾਂ ਨੂੰ ਬੁਲਾਏਗਾ ਅਤੇ ਪ੍ਰਬੰਧ ਕਰੇਗਾ।</li>
            <li>ਉਹ ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ ਅਤੇ ਐਸੋਸੀਏਸ਼ਨ ਦੀਆਂ ਮੀਟਿੰਗਾਂ ਦੇ ਮਿੰਟ ਰਿਕਾਰਡ ਕਰੇਗਾ।</li>
            <li>ਉਹ ਸਾਰੀਆਂ ਕਾਨੂੰਨੀ ਕਾਰਵਾਈਆਂ ਵਿੱਚ ਐਸੋਸੀਏਸ਼ਨ ਦੀ ਨੁਮਾਇੰਦਗੀ ਕਰੇਗਾ ਅਤੇ ਐਸੋਸੀਏਸ਼ਨ ਦੀ ਤਰਫੋਂ ਸਾਰੇ ਕਾਨੂੰਨੀ ਦਸਤਾਵੇਜ਼ਾਂ 'ਤੇ ਦਸਤਖਤ ਕਰੇਗਾ।</li>
            <li>ਉਹ ਮੈਂਬਰਾਂ ਅਤੇ ਸਾਰੀਆਂ ਗਾਹਕੀਆਂ ਅਤੇ ਦਾਨ ਦੇ ਇੱਕ ਨਵੀਨਤਮ ਰਜਿਸਟਰ ਨੂੰ ਕਾਇਮ ਰੱਖੇਗਾ।</li>
            <li>ਉਹ ਐਸੋਸੀਏਸ਼ਨ ਦੇ ਦਸਤਾਵੇਜਾਂ ਅਤੇ ਸਾਰੇ ਰਿਕਾਰਡ ਦਾ ਕਸਟੋਡੀਅਨ ਹੋਵੇਗਾ</li>
          </ol>
        </div>



        <div className="flex font-bold justify-between items-center acc my-2" onClick={() => filterDropdownHandler(fourthArrow, fourthList)}>
          <h1 className="font-bold mb-1 text-lg">ਸਕੱਤਰ</h1>
          <div ref={fourthArrow} className="arrow">
            <BiChevronDown className="h-6 w-6" />
          </div>
        </div>
        <div className="h-0 hidden acc-child text-gray-900 text-lg font-medium my-2" ref={fourthList}>
          <ol className=" list-inside list-decimal text-left space-x-4  text-justify">
            <li className="ml-4">ਉਹ ਪ੍ਰਬੰਧਕੀ ਮਾਮਲਿਆਂ ਵਿੱਚ ਜਨਰਲ ਸਕੱਤਰ ਦੀ ਮਦਦ ਕਰੇਗਾ।</li>
            <li>ਉਹ ਐਸੋਸੀਏਸ਼ਨ ਦੀਆਂ ਗਤੀਵਿਧੀਆਂ ਚਲਾਉਣ ਵਿੱਚ ਜਨਰਲ ਸਕੱਤਰ ਦੀ ਮਦਦ ਵੀ ਕਰਨਗੇ।</li>
          </ol>
        </div>


        <div className="flex font-bold justify-between items-center acc my-2" onClick={() => filterDropdownHandler(fifthArrow, fifthList)}>
          <h1 className="font-bold mb-1 text-lg ">ਸਹਾਇਕ ਸਕੱਤਰ ਅਤੇ ਖ਼ਜਾਨਚੀ</h1>
          <div ref={fifthArrow} className="arrow">
            <BiChevronDown className="h-6 w-6" />
          </div>
        </div>
        <div className="h-0 hidden acc-child text-gray-900 text-lg font-medium my-2" ref={fifthList}>
          <ol className=" list-inside list-decimal text-left space-x-4  text-justify">
            <li className="ml-4">ਉਹ ਐਸੋਸੀਏਸ਼ਨ ਦੇ ਵਿੱਤ/ਖਾਤਿਆਂ ਬਾਰੇ ਆਪਣੇ ਫਰਜ਼ਾਂ ਅਤੇ ਕਾਰਜਾਂ ਨੂੰ ਨਿਭਾਉਣ ਵਿੱਚ ਜਨਰਲ ਸਕੱਤਰ ਦੀ ਸਹਾਇਤਾ ਕਰੇਗਾ।</li>
            <li>ਉਹ ਅਜਿਹੇ ਹੋਰ ਕਰਤੱਵਾਂ ਅਤੇ ਕਾਰਜਾਂ ਨੂੰ ਨਿਭਾਏਗਾ ਜੋ ਕਾਰਜਕਾਰੀ ਕੌਂਸਲ ਦੁਆਰਾ ਉਸਨੂੰ ਸੌਂਪਿਆ ਜਾ ਸਕਦਾ ਹੈ।</li>
          </ol>
        </div>
      </div>






      <h1 className=" text-3xl text-black-900 text-center font-bold  mt-4">
      ਪੰਜਾਬੀ ਯੂਨੀਵਰਸਿਟੀ, ਪਟਿਆਲਾ ਦੇ ਸਾਬਕਾ ਵਾਈਸ ਚਾਂਸਲਰ
      </h1>
      <main className="grid justify-center  grid-cols-6 items-center  gap-2 py-2 px-4 my-10  bg-slate-500">
        {vcs.map((vca) => (

          <div className="
                          group
                          block  bg-white text-black overflow-hidden rounded-t-2xl shadow
                          hover:shadow-md
                          transition my-2  
                          ">
            <figure className="max-h-64 aspect-square overflow-hidden">

              <img
                className="w-full h-full object-cover  transition group-hover:scale-125"
                src={vca.image}
              />

            </figure>
            <div className="px-1 py-1">
              <h3 className="text-sm font-bold text-center">{vca.name}</h3>
              <p className="text-center text-sm">{vca.year}</p>
            </div>

          </div>
        ))}
      </main>







      <h1 className=" text-3xl text-black-900 text-center font-bold  mt-4">
      ਜ਼ਿਕਰਯੋਗ ਸਾਬਕਾ ਵਿਦਿਆਰਥੀ
      </h1>
      <Slider {...settings2} className="notable_alumni_slider mx-16 my-8 mb-16">
        {data.map((obj) => (
          <div className="flex flex-col items-center h-60 w-38 p-3  m-6 gap-6">
            <div className="shadow-xl ">
              <div className="bg-[#b8dded]  rounded-lg border border-slate-400  w-full">
                <img
                  className="h-56 w-56 p-6 rounded-full mx-auto "
                  src={obj.image}
                />
              </div>
              <div>
                <h1 className="pl-6 text-2xl  pt-2 pb-2">{obj.name}</h1>
              </div>
              <div>
                <p className="px-4 h-72  text-justify">{obj.description}</p>
              </div>
            </div>
          </div>
        ))}
      </Slider>
      <h1 className=" text-3xl text-black-900 text-center font-bold pb-8  mt-4">
      ਖੋਜ ਵਿਭਾਗ ਅਤੇ ਨੋਡਲ ਸੈਂਟਰ ਦੀ ਜਾਣਕਾਰੀ
      </h1>
      <div className=" flex justify-center">
              <button
                type="submit"
                className=" py-2 px-4 text-sm  mx-2 font-medium rounded-md text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
      <Link to="/research2">ਨੇਬਰਹੁੱਡ ਕੈਂਪਸ</Link>      
              </button>
              <button
                type="submit"
                className=" py-2 px-4 text-sm font-medium   mx-2 rounded-md text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
             <Link to="/research3">ਖੇਤਰੀ ਕੇਂਦਰ</Link> 
              </button>
              <button
                type="submit"
                className=" py-2 px-4 text-sm font-medium rounded-md  mx-2 text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
   <a href="/departments">ਅਧਿਆਪਨ ਅਤੇ ਖੋਜ ਵਿਭਾਗ</a>    
              </button>
              <button
                type="submit"
                className=" py-2 px-4 text-sm font-medium rounded-md  mx-2 text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
 <Link to="/research1">ਅੰਤਰ-ਅਨੁਸ਼ਾਸਨੀ ਖੋਜ ਵਿਭਾਗ</Link>   
              </button>
              <button
                type="submit"
                className=" py-2 px-4 text-sm font-medium rounded-md   mx-2 text-white bg-footercolor-700 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
     <Link to="/research4"> ਸੰਵਿਧਾਨਕ ਕਾਲਜ</Link>      
              </button>
            </div>
    </>
  );
};
export default HomePage;
